...
BEGIN
    ...
    SELECT * FROM (VALUES
      ...
      (1,'Hindi','व्यंजन परिचय','क ख ग घ - व्यंजन सीखें और शब्द बनाएं।','कबूतर कूऽ कूऽ करता है, क से क-ब-ऊ-तर!',20,
        ...
        '[{"q":"क के बाद?","a":"ग","b":"ख","c":"घ","d":"च","correct":"B"},
          {"q":"क से क्या?","a":"कमल","b":"आम","c":"इमली","d":"ऊंट","correct":"A"},
      ...
      (2,'EVS','Means of Transport','Cars, buses, trains, planes and ships help us travel.','From cycle to rocket — humans love to go places!',20,
  ...
  LOOP
    -- skip if a lesson with same grade+subject+topic exists
    IF EXISTS (SELECT 1 FROM public.lessons WHERE grade=rec.grade AND subject=rec.subject AND topic=rec.topic) THEN
      CONTINUE;
    END IF;
