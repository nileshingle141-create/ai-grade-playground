              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {p.lessons?.subject || ""} • {p.time_spent_minutes || 0} min
                  </p>
                </div>
                <div className="text-right">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-bold ${p.completed ? "bg-science/20 text-science" : "bg-muted text-muted-foreground"}`}
                  >
...
function Stat({
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4">
      <div
        className={`mb-2 inline-flex h-8 w-8 items-center justify-center rounded-lg ${color} text-white`}
      >
        {icon}
      </div>
